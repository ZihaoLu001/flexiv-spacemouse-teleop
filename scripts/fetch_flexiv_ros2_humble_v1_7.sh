...
if [ ! -d flexiv_ros2 ]; then
  ...
  cd flexiv_ros2
  # Refuse to clobber local modifications: the only sanctioned local change is
  # the Servo config installed by scripts/apply_servo_config.sh.
  dirty="$(git status --porcelain | grep -v 'rizon_moveit_servo_config.yaml' || true)"
  if [ -n "$dirty" ]; then
